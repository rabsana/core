...
class Jdf extends AbstractsDate
{
    ...
    /*    F    */
    public function jmktime($h = '', $m = '', $s = '', $jm = '', $jd = '', $jy = '')
    {
        $h = $this->tr_num($h);
        $m = $this->tr_num($m);
        $s = $this->tr_num($s);
    ...
    {
        $gy = (int) $this->tr_num($gy);
        $gm = (int) $this->tr_num($gm);
        $gd = (int) $this->tr_num($gd);/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
    ...
    public function gtoj($date, string $format = 'Y-m-d H:i:s'): string
    {
        try {
            ...
            $jalali_date = $this->gregorian_to_jalali($gregorian_date[0], $gregorian_date[1], $gregorian_date[2]);

            if (isset($gregorian_time)) {
                $timestamp = $this->jmktime($gregorian_time[0], $gregorian_time[1], $gregorian_time[2], (int) $jalali_date[1], (int) $jalali_date[2], (int) $jalali_date[0]);
            } else {
                $timestamp = $this->jmktime(0, 0, 0, (int) $jalali_date[1], (int) $jalali_date[2], (int) $jalali_date[0]);
            }

            $final_date = '';
            for ($p = 0; $p < sizeof($date_format); $p++) {
                $final_date .= $this->jdate($date_format[$p], $timestamp, '', 'Asia/Tehran', 'en');
                if ($p <= sizeof($date_format) - 2) {
                    $final_date .= $separators[$p];
                }
            }

            return (string) $final_date;

            // 
        } catch (Exception $e) {

            info("Date-gtoj-error: " . $e);
            return  '';
        }
    }
    ...
    public function jtog($date, string $format = 'Y-m-d H:i:s'): string
    {
        try {
            ...
            $gregorian_date = $this->jalali_to_gregorian($jalali_date[0], $jalali_date[1], $jalali_date[2]);
